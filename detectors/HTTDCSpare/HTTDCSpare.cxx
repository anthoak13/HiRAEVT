...
HTTDCSpare::HTTDCSpare(const char * name) :
HTDetector(name)
{
...
void HTTDCSpare::InitTTreeBranch(TTree * theTree)
{
  ...
  for(int SpareCh=0; SpareCh<fNumDetectors; SpareCh++) {
    HTTDCSpareChannel * newChannel = new HTTDCSpareChannel(((HTTDCSpareMap *)fDetectorMapping)->GetChannelName(SpareCh));
    fSpareChannels.push_back(newChannel);

    theTree->Branch(Form("%s.%s",fName.c_str(),fSpareChannels[SpareCh]->GetName()), (Double_t*)fSpareChannels[SpareCh]->GetValuePointer(),Form("%s.%s/D",fName.c_str(),fSpareChannels[SpareCh]->GetName()));
  }
  return;
}
