...
HTMCPMap::HTMCPMap(const std::string& name, int num_detectors) : HTDetectorMap(name),
...
HTMCPMap::~HTMCPMap()
{
  for(int j = 0; j < 2; ++j)
  {
    if(fEnergyModule[j]) delete [] fEnergyModule[j];
    if(fEnergyChannel[j]) delete [] fEnergyChannel[j];
    if(fTimeModule[j]) delete [] fTimeModule[j];
    if(fTimeChannel[j]) delete [] fTimeChannel[j];
  }
...
int HTMCPMap::ParseMapLine(const char * line_to_parse)
{
  ...
  LineStream >> detectorName >> anodeType >>
    chType >> detNumber >> moduleName >> moduleCh;
