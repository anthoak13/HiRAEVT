...
FragmentIndex::FragmentIndex() : m_frags() {}
 ...
 *
 */
FragmentIndex::FragmentIndex(uint16_t *data) : m_frags()
{
...
void FragmentIndex::indexFragments(uint16_t *begin, uint16_t *end)
{
   ...
   do {
      ...
      if ((data + dist) > end) {
         //      throw std::runtime_error("FragmentIndex::indexFragments() insufficient data in buffer for next
         //      fragment!");
      ...
      if (sizeBodyHeader == 0) {
         info.s_itembody = info.s_itemhdr + (sizeof(RingItemHeader) + 6) / sizeof(uint16_t);
         ;
      } else {
         info.s_itembody = info.s_itemhdr + (sizeof(RingItemHeader) + sizeBodyHeader) / sizeof(uint16_t);
      }
