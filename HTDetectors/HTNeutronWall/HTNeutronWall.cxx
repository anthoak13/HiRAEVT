...
HTNeutronWall::HTNeutronWall(const char *name, int num_bars) : HTDetector(name, num_bars), fNeutronWall(0), fevt(0)
{
...
void HTNeutronWall::BuildEvent()
{
   ...
   for (int NumBar = 0; NumBar < fNumDetectors; NumBar++) {
      fNeutronWall->GetBar(NumBar)->SetAnodeCommonLeft(
         dModuleLeft(NumBar) >= 0 ? ((HTRootCAEN7xx *)fModules[dModuleLeft(NumBar)])->GetData(dChannelLeft(NumBar))
                                  : -9999);
      fNeutronWall->GetBar(NumBar)->SetAnodeCommonRight(
         dModuleRight(NumBar) >= 0 ? ((HTRootCAEN7xx *)fModules[dModuleRight(NumBar)])->GetData(dChannelRight(NumBar))
                                   : -9999);
      fNeutronWall->GetBar(NumBar)->SetAnodeIndividualLeft(
         dModuleFastLeft(NumBar) >= 0
            ? ((HTRootCAEN7xx *)fModules[dModuleFastLeft(NumBar)])->GetData(dChannelFastLeft(NumBar))
            : -9999);
      fNeutronWall->GetBar(NumBar)->SetAnodeIndividualRight(
         dModuleFastRight(NumBar) >= 0
            ? ((HTRootCAEN7xx *)fModules[dModuleFastRight(NumBar)])->GetData(dChannelFastRight(NumBar))
            : -9999);
      fNeutronWall->GetBar(NumBar)->SetTimeLeft(
         dModuleTimeLeft(NumBar) >= 0
            ? ((HTRootCAEN1x90 *)fModules[dModuleTimeLeft(NumBar)])->GetData(dChannelTimeLeft(NumBar))
            : -9999);
      fNeutronWall->GetBar(NumBar)->SetTimeRight(
         dModuleTimeRight(NumBar) >= 0
            ? ((HTRootCAEN1x90 *)fModules[dModuleTimeRight(NumBar)])->GetData(dChannelTimeRight(NumBar))
            : -9999);
   }
...
void HTNeutronWall::FillMappedData()
{
   ...
   for (int NumBar = 0; NumBar < fNumDetectors; NumBar++) {
      ...
      if (BarToFill->GetAnodeCommonLeft() > 0 && BarToFill->GetAnodeCommonRight() > 0 &&
          BarToFill->GetTimeLeft() != -9999 && BarToFill->GetTimeRight() != -9999) {
         ...
         fevt->fNeutronWall.fGeoMean[fevt->fNeutronWall.fmulti] =
            sqrt(BarToFill->GetAnodeCommonLeft() * BarToFill->GetAnodeCommonRight());
         fevt->fNeutronWall.ffastGeoMean[fevt->fNeutronWall.fmulti] =
            BarToFill->GetAnodeIndividualLeft() > 0 && BarToFill->GetAnodeIndividualRight() > 0
               ? sqrt(BarToFill->GetAnodeIndividualLeft() * BarToFill->GetAnodeIndividualRight())
               : 0;
