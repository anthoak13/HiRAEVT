...
HTNeutronWallMap::HTNeutronWallMap(const std::string &name, int num_bars)
   : HTDetectorMap(name), fNumBars(num_bars), fLeftModule(0), fLeftChannel(0), fLeftFastModule(0), fLeftFastChannel(0),
     fRightModule(0), fRightChannel(0), fRightFastModule(0), fRightFastChannel(0), fLeftTimeModule(0),
     fLeftTimeChannel(0), fRightTimeModule(0), fRightTimeChannel(0)
{
...
HTNeutronWallMap::~HTNeutronWallMap()
{
   ...
   if (fLeftChannel)
      delete[] fLeftChannel;
   if (fLeftFastModule)
      delete[] fLeftFastModule;
   if (fLeftFastChannel)
      delete[] fLeftFastChannel;
   if (fRightModule)
      delete[] fRightModule;
   if (fRightChannel)
      delete[] fRightChannel;
   if (fRightFastModule)
      delete[] fRightFastModule;
   if (fRightFastChannel)
      delete[] fRightFastChannel;
   if (fLeftTimeModule)
      delete[] fLeftTimeModule;
   if (fLeftTimeChannel)
      delete[] fLeftTimeChannel;
   if (fRightTimeModule)
      delete[] fRightTimeModule;
   if (fRightTimeChannel)
      delete[] fRightTimeChannel;
}
