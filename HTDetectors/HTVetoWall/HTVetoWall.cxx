...
HTVetoWall::HTVetoWall(const char *name, int num_bars) : HTDetector(name, num_bars), fVetoWall(0), fevt(0)
{
...
void HTVetoWall::BuildEvent()
{
   ...
   for (int NumBar = 0; NumBar < fNumDetectors; NumBar++) {
      fVetoWall->GetBar(NumBar)->SetAnodeCommonBottom(
         dModuleBottom(NumBar) >= 0
            ? ((HTRootCAEN7xx *)fModules[dModuleBottom(NumBar)])->GetData(dChannelBottom(NumBar))
            : -9999);
      fVetoWall->GetBar(NumBar)->SetAnodeCommonTop(
         dModuleTop(NumBar) >= 0 ? ((HTRootCAEN7xx *)fModules[dModuleTop(NumBar)])->GetData(dChannelTop(NumBar))
                                 : -9999);
      fVetoWall->GetBar(NumBar)->SetTimeBottom(
         dModuleTimeBottom(NumBar) >= 0
            ? ((HTRootCAEN1x90 *)fModules[dModuleTimeBottom(NumBar)])->GetData(dChannelTimeBottom(NumBar))
            : -9999);
      fVetoWall->GetBar(NumBar)->SetTimeTop(
         dModuleTimeTop(NumBar) >= 0
            ? ((HTRootCAEN1x90 *)fModules[dModuleTimeTop(NumBar)])->GetData(dChannelTimeTop(NumBar))
            : -9999);
   }
...
void HTVetoWall::FillMappedData()
{
   ...
   for (int NumBar = 0; NumBar < fNumDetectors; NumBar++) {
      ...
      if (BarToFill->GetAnodeCommonBottom() > 0 && BarToFill->GetAnodeCommonTop() > 0 &&
          BarToFill->GetTimeBottom() != -9999 && BarToFill->GetTimeTop() != -9999) {
         ...
         fevt->fVetoWall.fGeoMean[fevt->fVetoWall.fmulti] =
            sqrt(BarToFill->GetAnodeCommonBottom() * BarToFill->GetAnodeCommonTop());
