...
HTVetoWallMap::HTVetoWallMap(const std::string &name, int num_bars)
   : HTDetectorMap(name), fNumBars(num_bars), fBottomModule(0), fBottomChannel(0), fTopModule(0), fTopChannel(0),
     fBottomTimeModule(0), fBottomTimeChannel(0), fTopTimeModule(0), fTopTimeChannel(0)
{
...
{
   if (fBottomModule)
      delete[] fBottomModule;
   if (fBottomChannel)
      delete[] fBottomChannel;
   if (fTopModule)
      delete[] fTopModule;
   if (fTopChannel)
      delete[] fTopChannel;
   if (fBottomTimeModule)
      delete[] fBottomTimeModule;
   if (fBottomTimeChannel)
      delete[] fBottomTimeChannel;
   if (fTopTimeModule)
      delete[] fTopTimeModule;
   if (fTopTimeChannel)
      delete[] fTopTimeChannel;
}
