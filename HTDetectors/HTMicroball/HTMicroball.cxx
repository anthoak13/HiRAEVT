...
HTMicroball::HTMicroball(const char *name)
   : HTDetector(name), fMicroball(0), fevt(0), fNumDetectors(TOT_NUMER_MICROBALL_DETECTORS)
{
...
void HTMicroball::BuildEvent()
{
   ...
   for (int NumRing = 0; NumRing < N_MICROBALL_RINGS; NumRing++) {
      ...
      for (int NumDet = 0; NumDet < N_MICROBALL_DETS_PER_RING[NumRing]; NumDet++) {
         ...
         currDet->SetTail(
            dModuleTail(NumRing, NumDet) >= 0
               ? ((HTRootCAEN7xx *)fModules[dModuleTail(NumRing, NumDet)])->GetData(dChannelTail(NumRing, NumDet))
               : -9999);
         currDet->SetFast(
            dModuleFast(NumRing, NumDet) >= 0
               ? ((HTRootCAEN7xx *)fModules[dModuleFast(NumRing, NumDet)])->GetData(dChannelFast(NumRing, NumDet))
               : -9999);
         currDet->SetTime(
            dModuleTime(NumRing, NumDet) >= 0
               ? ((HTRootCAEN7xx *)fModules[dModuleTime(NumRing, NumDet)])->GetData(dChannelTime(NumRing, NumDet))
               : -9999);
      }
   }
   ...
   for (int NumRing = 0; NumRing < fMicroball->GetNumberRings(); NumRing++) {
      ...
      for (int NumDet = 0; NumDet < N_MICROBALL_DETS_PER_RING[NumRing]; NumDet++) {
         ...
         if ((DetToFill->GetTail() > 0 || DetToFill->GetFast() > 0) && DetToFill->GetTime() > 0) {
            ...
            fevt->fMicroball.fnumdet[fevt->fMicroball.fmulti] =
               NumDet + 1; // WARNING: detectors are counted from 1 in each ring
