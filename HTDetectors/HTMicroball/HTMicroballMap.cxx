...
HTMicroballMap::HTMicroballMap(const std::string &name)
   : HTDetectorMap(name), fFastModule(0), fFastChannel(0), fTailModule(0), fTailChannel(0), fTimeModule(0),
     fTimeChannel(0)
{
...
HTMicroballMap::~HTMicroballMap()
{
   for (int i = 0; i < N_RINGS; i++) {
      if (fFastModule[i])
         delete[] fFastModule[i];
      if (fFastChannel[i])
         delete[] fFastChannel[i];
      if (fTailModule[i])
         delete[] fTailModule[i];
      if (fTailChannel[i])
         delete[] fTailChannel[i];
      if (fTimeModule[i])
         delete[] fTimeModule[i];
      if (fTimeChannel[i])
         delete[] fTimeChannel[i];
   }
   if (fFastModule)
      delete[] fFastModule;
   if (fFastChannel)
...
int HTMicroballMap::ParseMapLine(const char *line_to_parse)
{
   ...
   int RingNumber = std::stoi(DetectorToSet.substr(DetectorToSet.find("R") + 1,
                                                   DetectorToSet.find("_") - (DetectorToSet.find("R") + 1))) -
                    1;
