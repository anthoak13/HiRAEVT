...
HTMicroballRing::HTMicroballRing(Int_t ring_number, Int_t number_of_detectors)
   : fNumRing(ring_number), fNumDetectors(number_of_detectors)
{
...
{
   if (!fMicroballDetector)
      return;
   for (int i = 0; i < fNumDetectors; i++) {
      if (fMicroballDetector[i])
         delete fMicroballDetector[i];
   }

   if (!fMicroballDetector)
      delete[] fMicroballDetector;
}
