...
HTForwardArray::HTForwardArray(const char *name, int num_detectors)
   : HTDetector(name, num_detectors), fForwardArray(0), fevt(0)
{
...
void HTForwardArray::BuildEvent()
{
   ...
   for (int NumDet = 0; NumDet < fNumDetectors; NumDet++) {
      fForwardArray->GetDetector(NumDet)->SetE(
         dModuleEnergy(NumDet) >= 0
            ? ((HTRootCAEN7xx *)fModules[dModuleEnergy(NumDet)])->GetData(dChannelEnergy(NumDet))
            : -9999);
      fForwardArray->GetDetector(NumDet)->SetTime(
         dModuleTime(NumDet) >= 0 ? ((HTRootCAEN1x90 *)fModules[dModuleTime(NumDet)])->GetData(dChannelTime(NumDet))
                                  : -9999);
   }
...
void HTForwardArray::FillMappedData()
{
   ...
   for (int NumDet = 0; NumDet < fNumDetectors; NumDet++) {
      ...
      if (DetToFill->GetE() > 0 && DetToFill->GetTime() != -9999) {
         fevt->fForwardArray.fnumdet[fevt->fForwardArray.fmulti] =
            NumDet + 1; // WARNING: detectors are counted from 1 in the FA experimental configuration
