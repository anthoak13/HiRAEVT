...
HTForwardArrayCluster::HTForwardArrayCluster(int number_of_detectors) : fNumDetectors(number_of_detectors)
{
...
{
   if (!fForwardArrayDetector)
      return;
   for (int i = 0; i < fNumDetectors; i++) {
      if (fForwardArrayDetector[i])
         delete fForwardArrayDetector[i];
   }

   if (!fForwardArrayDetector)
      delete[] fForwardArrayDetector;
}
