...
HTHiRAMap::HTHiRAMap(const std::string &name, int num_telescopes)
   : HTDetectorMap(name), fNumTelescopes(num_telescopes), fNumStripsperTelescope(N_STRIPS_PER_TELESCOPE),
     fNumCsIperTelescope(N_CSIS_PER_TELESCOPE), fCsIEnergyModule(0), fCsIEnergyChannel(0), fCsITimeModule(0),
     fCsITimeChannel(0), fModuleEF(0), fModuleEB(0), fStripEF(0), fStripEB(0)
{
...
HTHiRAMap::~HTHiRAMap()
{
   if (fCsIEnergyModule) {
      for (int i = 0; i < fNumTelescopes; i++) {
         if (fCsIEnergyModule[i])
            delete[] fCsIEnergyModule[i];
      }
      delete fCsIEnergyModule;
   }
   if (fCsIEnergyChannel) {
      for (int i = 0; i < fNumTelescopes; i++) {
         if (fCsIEnergyChannel[i])
            delete[] fCsIEnergyChannel[i];
      }
   ...
   if (fCsITimeModule) {
      for (int i = 0; i < fNumTelescopes; i++) {
         if (fCsITimeModule[i])
            delete[] fCsITimeModule[i];
      }
      delete fCsITimeModule;
   }
   if (fCsITimeChannel) {
      for (int i = 0; i < fNumTelescopes; i++) {
         if (fCsITimeChannel[i])
            delete[] fCsITimeChannel[i];
      }
...
int HTHiRAMap::ParseMapLine(const char *line_to_parse)
{
   ...
   DetectorToSet.assign(
      DetectorToSet.substr(DetectorToSet.find("detector=\"") + 10,
                           DetectorToSet.find_last_of("\"") - (DetectorToSet.find("detector=\"") + 10)));
   ModuleName.assign(ModuleName.substr(ModuleName.find("module=\"") + 8,
                                       ModuleName.find_last_of("\"") - (ModuleName.find("module=\"") + 8)));
   ...
   if (ChName.compare("EF") == 0 || ChName.compare("EB") == 0) {
      ...
      if (ChName.compare("EF") == 0)
         fModuleEF[TelNumber] = ModuleNumber;
      if (ChName.compare("EF") == 0)
         fModuleEB[TelNumber] = ModuleNumber;
      ...
      MotherBoardName.assign(
         MotherBoardName.substr(MotherBoardName.find("motherboard=\"") + 13,
                                MotherBoardName.find_last_of("\"") - (MotherBoardName.find("motherboard=\"") + 13)));
      ChipBoardName.assign(
         ChipBoardName.substr(ChipBoardName.find("chipboard=\"") + 11,
                              ChipBoardName.find_last_of("\"") - (ChipBoardName.find("chipboard=\"") + 11)));
      ...
      for (int NChip = 0; NChip < N_HINP_CHIP_PER_CHIPBOARD; NChip++) {
         for (int NChannel = 0; NChannel < N_HINP_CHANNELS_PER_CHIP; NChannel++) {
            ...
            //   (...)
            if (ChName.compare("EF") == 0)
               fStripEF[TelNumber][MotherBoardNumber][AbsoluteNChip - 1][NChannel] =
                  NChannel * 2 * (1 - NChip % 2) + (NChip % 2) * (2 * NChannel + 1);
            if (ChName.compare("EB") == 0)
               fStripEB[TelNumber][MotherBoardNumber][AbsoluteNChip - 1][NChannel] =
                  NChannel * 2 * (1 - NChip % 2) + (NChip % 2) * (2 * NChannel + 1);
         }
      }
      ...
      int TelNumber = std::stoi(DetectorToSet.substr(DetectorToSet.find("TEL") + 3,
                                                     DetectorToSet.find("_") - (DetectorToSet.find("TEL") + 3)));
      ...
      ModuleChannelString.assign(ModuleChannelString.substr(ModuleChannelString.find("channel=\"") + 9,
                                                            ModuleChannelString.find_last_of("\"") -
                                                               (ModuleChannelString.find("channel=\"") + 9)));
