...
HTHiRA::HTHiRA(const char *name, int num_telescopes) : HTDetector(name, num_telescopes), fHiRACsI(0), fevt(0)
{
...
void HTHiRA::BuildEvent()
{
   ...
   for (int NumTel = 0; NumTel < fNumDetectors; NumTel++) {
      for (int NumCsI = 0; NumCsI < fHiRACsI->GetNumCsI(); NumCsI++) {
         ...
         currCsI->SetEnergy(
            dModuleEnergy(NumTel, NumCsI) >= 0
               ? ((HTRootCAEN7xx *)fModules[dModuleEnergy(NumTel, NumCsI)])->GetData(dChannelEnergy(NumTel, NumCsI))
               : -9999);
         currCsI->SetTime(
            dModuleTime(NumTel, NumCsI) >= 0
               ? ((HTRootCAEN1x90 *)fModules[dModuleTime(NumTel, NumCsI)])->GetData(dChannelTime(NumTel, NumCsI))
               : -9999);
      }
   }
...
void HTHiRA::FillMappedData()
{
   ...
   for (int NumTel = 0; NumTel < fNumDetectors; NumTel++) {
      ...
      TTreeReaderValue<HTHINPHit> *HINPDataEF =
         dModuleEF(NumTel) >= 0 ? ((HTRootHINP *)fModules[dModuleEF(NumTel)])->GetDataPointer() : 0;
      fevt->fHiRA[NumTel].fEF.fmulti = 0;
      for (int HINPhit = 0; HINPhit < (*(*HINPDataEF)).fNHits; HINPhit++) {
         int StripToSet = dStripEF(NumTel, (*(*HINPDataEF)).fBank[HINPhit], (*(*HINPDataEF)).fChip[HINPhit],
                                   (*(*HINPDataEF)).fChannel[HINPhit]);
         if (StripToSet < 0)
            continue;
         ...
         if (fevt->fHiRA[NumTel].fEF.fmulti == N_STRIPS_PER_TELESCOPE)
            break;
         // NOTE: this condition prevents from having multiplicities larger than the number of actual strips, in the
         // presence of corrupted buffers.
      }
      TTreeReaderValue<HTHINPHit> *HINPDataEB =
         dModuleEB(NumTel) >= 0 ? ((HTRootHINP *)fModules[dModuleEB(NumTel)])->GetDataPointer() : 0;
      fevt->fHiRA[NumTel].fEB.fmulti = 0;
      for (int HINPhit = 0; HINPhit < (*(*HINPDataEB)).fNHits; HINPhit++) {
         int StripToSet = dStripEB(NumTel, (*(*HINPDataEB)).fBank[HINPhit], (*(*HINPDataEB)).fChip[HINPhit],
                                   (*(*HINPDataEB)).fChannel[HINPhit]);
         if (StripToSet < 0)
            continue;
         ...
         if (fevt->fHiRA[NumTel].fEB.fmulti == N_STRIPS_PER_TELESCOPE)
            break;
         // NOTE: this condition prevents from having multiplicities larger than the number of actual strips, in the
         // presence of corrupted buffers.
      }
