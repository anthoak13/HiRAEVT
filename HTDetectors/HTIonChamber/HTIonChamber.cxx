...
HTIonChamber::HTIonChamber(const char *name, int num_detectors) : HTDetector(name, num_detectors), fevt(0)
{
...
void HTIonChamber::BuildEvent()
{
   ...
   for (int detNum = 0; detNum < fNumDetectors; detNum++) {
      ...
      if (map->GetEnergyModule(detNum) >= 0)
         fevt->fIonChamber.fE[detNum] =
            ((HTRootMADC32 *)fModules[map->GetEnergyModule(detNum)])->GetData(map->GetEnergyChannel(detNum));
      else
      ...
      if (map->GetTimeModule(detNum) >= 0)
         fevt->fIonChamber.fTime[detNum] =
            ((HTRootCAEN1x90 *)fModules[map->GetTimeModule(detNum)])->GetData(map->GetTimeChannel(detNum));
      else
