...
HTHINPUnpacker::HTHINPUnpacker(const char *name, Int_t nMBs, Bool_t usingSISFADC)
   : fFoundBeginMarker(0), fReportNExtraMarkers(0), fTotalUnpackedData(0), fFaultyCount(0), fErrorCount(0),
     fChName(name)
{
   // --
   //
...
//______________________________________________________________________________
void HTHINPUnpacker::Clear(Option_t *option)
{
   // --
   //
...
//______________________________________________________________________________
void HTHINPUnpacker::InitBranch(TTree *tree)
{
   // --
   //
   if (GetFillData()) {
      tree->Branch(fChName + ".", "HTHINPHit", &fHits, 32768, 2);
   } else {
      cout << "-->HTHINPUnpacker::InitBranch  Branches will not be created or filled." << endl;
   }
}

//______________________________________________________________________________
void HTHINPUnpacker::InitTree(TTree *tree)
{
   // --
   //
...
//______________________________________________________________________________
void HTHINPUnpacker::AddHit(Int_t xlm, UShort_t bank, Bool_t mismatch, UShort_t ch, UShort_t chip, UShort_t Hi,
                            UShort_t Lo, UShort_t time)
{
...
//______________________________________________________________________________
Int_t HTHINPUnpacker::Unpack(vector<UShort_t> &event, UInt_t offset)
{
   ...
   //  cout << "Starting HTHINPUnpacker::Unpack" << endl;
   ...
   cout << hex << event[offset+i] << endl;
      ...
      */
      ...
      if (channelCount > 4095) {
         fErrorCount++;
         cerr << "Got an error report from " << hex << channelCount << dec << endl;
         //    offset += wordCount-1;
      ...
      for (unsigned int i = 0; i < channelCount / 4; i++) {
         ...
         if (chip > 0 && chip <= numChips) {
            // printf("chip %d board %d chan %d Hi %d Lo %d Time %d
            // (numChips=%d)\n",chip,board,channel,Hi,Lo,time,numChips); cout <<"Board = " << board-1 << endl;
            ...
            */
            ...
            // cout <<"chip = " << dec << chip <<" channel = " << channel << " " << Hi << " " << Lo << " " << time <<
            // endl;
         } else {
            bumflag = 1;
            fFaultyCount++;
            printf("faulty chip %d board %d chan %d A %d B %d Time %d (numChips=%d)\n", chip, board, channel, Hi, Lo,
                   time, numChips);
         }
      }
...
//______________________________________________________________________________
void HTHINPUnpacker::PrintSummary()
{
   printf("-- module %s --\n", fChName.Data());
...
//______________________________________________________________________________
void HTHINPUnpacker::AddTTreeUserInfo(TTree *tree)
{
   TNamed *unpackedData =
      new TNamed(Form("module %s : Total Unpacked Data", fChName.Data()), Form("%llu", fTotalUnpackedData));
   TNamed *errorsChipFound =
      new TNamed(Form("module %s : Faulty Chip Errors Found", fChName.Data()), Form("%llu", fFaultyCount));
   TNamed *errorsBufFound =
      new TNamed(Form("module %s : Buffer Errors Found", fChName.Data()), Form("%llu", fErrorCount));
