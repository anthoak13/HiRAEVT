...
**
*/
unsigned int CHINPA::operator()(CEvent &rEvent, vector<unsigned short> &event, unsigned int offset,
                                CParamMapCommand::AdcMapping *pMap)
{
   ...
   for (int bank = 0; bank < nMBs; bank++) {
      ...
      if (channelCount > 4095) {
         cerr << "Got an error report from " << pMap->name << " " << hex << channelCount << dec << endl;
         //    offset += wordCount-1;
...
// CHINPA::getTree(CParamMapCommand::AdcMapping* pMap,
//		CEvent& rEvent)
CTreeParameterArray ****CHINPA::getTree(CParamMapCommand::AdcMapping *pMap, CEvent &rEvent)
{
   if (!pMap->extraData) {
      SpecTcl *api = SpecTcl::getInstance();
      CTCLInterpreter *pInterp = api->getInterpreter();
      ...
      for (int mb = 0; mb < nMBs; mb++) {
         ...
         for (int i = 0; i < chips.size(); i++) {
            ...
            teArray[0] = new CTreeParameterArray(eBaseName, 14, 32, 0);
            string eLoBaseName = baseName;
            eLoBaseName += ".";
            eLoBaseName += string(bname);
            eLoBaseName += ".eLo.";
            eLoBaseName += chipNumber;
            teArray[1] = new CTreeParameterArray(eLoBaseName, 14, 32, 0);
            string tBaseName = baseName;
            eBaseName += ".";
            eBaseName += string(bname);
            tBaseName += ".t.";
            tBaseName += chipNumber;
            teArray[2] = new CTreeParameterArray(tBaseName, 14, 32, 0);
         }
