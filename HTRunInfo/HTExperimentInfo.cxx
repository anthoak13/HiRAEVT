...
HTExperimentInfo::HTExperimentInfo() : fDataMerged(false), fFirstRun(0), fLastRun(0), _runNum(-1), _runInfo(nullptr) {}
...
HTRunInfo *HTExperimentInfo::SetRunNumber(int run_num)
{
   ...
   success &= newRunInfo->LoadDAQSettings(fDAQConfigurationFileName[run_num - fFirstRun].c_str()) > 0;

   success &= newRunInfo->SetPedestalsFile(fPedestalFileName[run_num - fFirstRun].c_str()) == 0;

   success &= newRunInfo->SetMappingFile(fMappingFileName[run_num - fFirstRun].c_str()) == 0;
...
void HTExperimentInfo::ParseSetConfigLine(const char *line_to_parse)
{
   ...
   std::string NewValue(
      LineToParse.substr(LineToParse.find("\"") + 1, LineToParse.find_last_of("\"") - (LineToParse.find("\"") + 1)));
   ...
   if (ValueToSet.compare("EXPERIMENT_NAME") == 0) {
      ...
      if (LineToParse.find("--run") ==
          std::string::npos) { // EvtFileName might be different run-by-run, if --run is specified don't set it now but
                               // in ParseSetConfigLineRunInfo
      ...
      if (Option.find("--run=") != std::string::npos) {
         ...
         if (Option.find(",") != std::string::npos) {
            ...
            while (std::getline(LineRunStream, RunToInclude, ',')) {
               if (run_num == std::stoi(RunToInclude))
                  RunFound = true;
            }
         }
         if (Option.find("-") != std::string::npos) {
         ...
         if (Option.find(",") != std::string::npos) {
            ...
            while (std::getline(LineExcludeStream, RunToExclude, ',')) {
               if (run_num == std::stoi(RunToExclude))
                  return; // this run is excluded
            }
         }
         if (Option.find("-") != std::string::npos) {
            std::string StartRun;
            ...
            int StopRunNum = std::stoi(StopRun);
            if (run_num >= StartRunNum && run_num <= StopRunNum)
               return; // this run belong to a bunch of runs to be excluded
         }
         if (Option.find(",") == std::string::npos && Option.find("-") == std::string::npos && !Option.empty()) {
            if (run_num == std::stoi(Option))
               return;
...
int HTExperimentInfo::RetrieveRunTitlesFromDatabaseFile()
{
   ...
   while (!FileIn.eof()) {
      ...
      if (LineRead.find('*') == 0)
         continue;
      if (LineRead.find_first_not_of(' ') == std::string::npos)
         continue;
      ...
      if (run_num >= fFirstRun && run_num <= fLastRun) {
         NRead++;
         fRunTitle[run_num - fFirstRun].assign(
            LineRead.substr(LineRead.find_first_not_of(' ', LineRead.find_first_of(' '))));
      }
   }
