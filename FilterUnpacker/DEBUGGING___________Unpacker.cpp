...
void Unpacker::operator()(FragmentIndex& index, uint32_t totalSize, uint64_t eventTimestamp)
{
  ...
  if(fCounter == 50){
    ...
    if (fPercentDone<22.99) {
       printf("skip event\n");
      fReadWords += totalSize;
      return;
    }
    else printf("I'm in the event %ld\n",nevent);
...
//DEBUG         
//     if(fPercentDone<51.99){
      //don't unpack this event
//       fExperiment->Fill(1);
//       fReadWords += totalSize;
//       return; 
//     }
//     printf("sto impiattando\n");  
    if (fPercentDone<22.99) {
       printf("skip event\n");
      fReadWords += totalSize;
      getchar();
  ...
  while(RBDetector *det = (RBDetector*)nextDet()){
    ...
    if(det->GetAnalysisEnabled()){
      // calibrations and physical analysis of the detector
      det->AnalyzeDetector();
    }
  }
